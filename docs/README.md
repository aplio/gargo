...
- `[ui] respect_gitignore = true` additionally hides what git ignores (`target/`, `build/`) from the file tree, bringing it in line with the pickers, whose list comes from git and already skips them. Off by default, and a separate switch from `show_dotfiles`: "hidden" and "ignored" are different ideas. The ignore set is queried per directory listing rather than cached — a rule that has just been deleted must stop hiding files immediately.
- Dotfiles (`.github/`, `.claude/`, `.env`) are listed in the file tree and the file pickers by default; `.git` itself never is. `.` in the explorer sidebar (or the `Toggle Hidden Files` palette command) flips it for the session — tree, big explorer and the `SPC f` picker move together. `[ui] show_dotfiles = false` in `config.toml` hides them from the start.
...
- `[theme]` -- preset (`ansi_dark` / `ansi_light` / `gargo_dark` / `gargo_light` / `gargo_dim` / `gargo_contrast` / `gargo_sepia`), capture overrides, and `[theme.ui]` role overrides
...
- `config.toggle_wrap` flips `wrap` (palette label "Enable/Disable Line Wrap")
