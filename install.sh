  ...
  Linux)
    case "${arch}" in
      ...
      aarch64|arm64)
        ...
        ;;
      *)
        echo "Unsupported Linux architecture: ${arch}. Only x86_64 and aarch64 are supported." >&2
        exit 1
        ;;
    esac
    ;;
  Darwin)
    case "${arch}" in
      ...
      arm64|aarch64)
        ...
        ;;
      *)
        echo "Unsupported macOS architecture: ${arch}. Only x86_64 and aarch64 are supported." >&2
        exit 1
        ;;
    esac
    ;;
...
if [ -n "${GARGO_VERSION:-}" ]; then
  release_api="${GITHUB_API}/tags/v${GARGO_VERSION#v}"
else
  release_api="${GITHUB_API}/latest"
fi
verify_checksums=1
if [ "${GARGO_SKIP_VERIFY:-0}" = "1" ]; then
  verify_checksums=0
fi
...
archive_info="$(extract_asset "gargo-.*-${asset_suffix}\\.tar\\.gz$")"
archive="${archive_info%%|*}"
archive_url="${archive_info##*|}"

if [ -z "${archive}" ] || [ "${archive}" = "${archive_info}" ]; then
  echo "Could not locate a gargo ${asset_suffix} archive in release ${tag}." >&2
  exit 1
fi
...
install_dir="${GARGO_BIN_DIR:-$HOME/.local/bin}"
tmpdir="$(mktemp -d)"
cleanup() {
  rm -rf "${tmpdir}"
}
...
if [ "${verify_checksums}" = "1" ]; then
  ...
  if command -v sha256sum >/dev/null 2>&1; then
    ...
    echo "Set GARGO_SKIP_VERIFY=1 to skip verification." >&2
    exit 1
  fi
...
if [ ! -x "${tmpdir}/gargo" ]; then
  echo "Downloaded archive did not contain a valid gargo binary." >&2
  exit 1
fi

mkdir -p "${install_dir}"
install_path="${install_dir}/gargo"
cp "${tmpdir}/gargo" "${install_path}"
chmod +x "${install_path}"

echo "Installed gargo ${tag} to ${install_path}"
